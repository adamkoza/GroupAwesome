...
class MovieDB
{
    ...
    public static void main(String [] args){
        ...
        EHMTable movie = new EHMTable("movie", "title year length genre studioName producerNo",
                "String Integer Integer String String Integer", "title year");

        EHMTable cinema = new EHMTable("cinema", "title year length genre studioName producerNo",
                "String Integer Integer String String Integer", "title year");

        EHMTable movieStar = new EHMTable("movieStar", "name address gender birthdate",
                "String String Character String", "name");

        EHMTable starsIn = new EHMTable("starsIn", "movieTitle movieYear starName",
                "String Integer String", "movieTitle movieYear starName");

        EHMTable movieExec = new EHMTable("movieExec", "certNo name address fee",
                "Integer String String Float", "certNo");

        EHMTable studio = new EHMTable("studio", "name address presNo",
        ...
        out.println ();
        EHMTable t_project = movie.project ("title year");
        t_project.print ();

        //--------------------- select

        out.println ();
        EHMTable t_select = movie.select (t -> t[movie.col("title")].equals ("Star_Wars") &&
        ...
        out.println ();
        EHMTable t_iselect = movieStar.select (new KeyType ("Harrison_Ford"));
        t_iselect.print ();

        //--------------------- union

        out.println ();
        EHMTable t_union = movie.union (cinema);
        t_union.print ();

        //--------------------- minus

        out.println ();
        EHMTable t_minus = movie.minus (cinema);
        t_minus.print ();
        ...
        EHMTable t_join = movie.join ("studioName", "name", studio);
        t_join.print ();

        movie.print();
        cinema.print();
        out.println ();
        EHMTable t_join2 = movie.join ("title year", "title year", cinema);
        t_join2.print ();
